...
class PlayState extends MusicBeatState
{
	...
	public var noteKillOffset:Float = 1500;
	...
	public static var FreeplayCurSelected:Int = 1;
    public static var ExtraFreeplayCurSelected:Int = 1;
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			for (songNotes in section.sectionNotes)
			{
				...
				susLength = (susLength / Conductor.stepCrochet) - 1;
		...
		if(ret != FunkinLua.Function_Stop && !transitioning) {
			...
			else
			{
				...
				if(FlxTransitionableState.skipNextTransIn) {
				    ...
				    if (PlayState.SONG.song == 'setting')
				    {
		...
		if (startedCountdown && !boyfriend.stunned && generatedMusic)
		{
			...
			notes.forEachAlive(function(daNote:Note)
			{
				...
				&& daNote.mustPress && !daNote.tooLate && !daNote.wasGoodHit && !daNote.blockHit) {
					...
					anas[daNote.noteData].hitJudge = Ratings.judgeNote(noteDiff);
	...
	function noteMiss(direction:Int = 1, daNote:Note):Void { //You didn't hit the key and let it go offscreen, also used by Hurt Notes
		//Dupe note remove
		notes.forEachAlive(function(note:Note) {
		...
		if (daNote != null)
			{
				if (!loadRep)
				{
					saveNotes.push([
						...
						0,
						direction,
						-(166 * Math.floor((Conductor.safeZoneOffset / 60) * 1000) / 166)
					]);
					saveJudge.push("miss");
				}
			}
	...
	function opponentNoteHit(note:Note):Void
	{
		...
		if (SONG.needsVoices && songScore < 1)
		{
			vocals.volume = 1;
        }
		var time:Float = 0.15;
	...
	public function RecalculateRating(badHit:Bool = false) {
		...
		if(ret != FunkinLua.Function_Stop)
		{
			...
			// Rating FC
			ratingFC = "";
			if (sicks > 0 || marvelouss > 0) ratingFC = "SFC";
